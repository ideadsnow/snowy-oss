    ...
    try {
      ...
      if (typeof window !== "undefined" && (window as any).__TAURI__) {
        try {
          ...
          console.warn(
            "Tauri fullscreen failed, falling back to web API:",
            tauriError
          );
        }
      }
    ...
    <Dialog open={showImagePreview} onOpenChange={setShowImagePreview}>
      <DialogContent className="max-w-[98vw] max-h-[98vh] p-0 bg-black border-none">
        ...
        >
          ...
          {notifications && (
            <div
              className={`absolute top-4 left-1/2 transform -translate-x-1/2 z-50 px-4 py-2 rounded-lg ${
          ...
          {/* 顶部工具栏 */}
          <div
            className={`absolute top-0 left-0 right-0 z-40 bg-black/80 p-3 flex justify-between items-center transition-transform duration-300 ${
          ...
          {imageLoaded && showToolbar && (
            <div
              className={`absolute bottom-0 left-0 right-0 bg-black/80 p-2 text-center transition-transform duration-300 ${
                showToolbar ? "translate-y-0" : "translate-y-full"
              }`}
            >
